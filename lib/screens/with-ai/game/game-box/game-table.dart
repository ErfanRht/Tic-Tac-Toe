...
class _GameTableState extends State<GameTable> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<GameAiController>(
      builder: (state) {
        return Stack(
          children: [
            Row(
  ...
  setWinner(BoxValue value) async {
    WinnerMode winner;
    Get.find<GameAiController>().updateGame(newBoxStatus: IntoBox.RESULT);
    if (value == BoxValue.CROSS) {
      await Future.delayed(Duration(milliseconds: 1500));
      winner = WinnerMode.USER;
      Get.find<GameAiController>().addUserWins();
    } else if (value == BoxValue.ZERO) {
      await Future.delayed(Duration(milliseconds: 1500));
      winner = WinnerMode.AI;
      Get.find<GameAiController>().addAiWins();
    } else {
      await Future.delayed(Duration(milliseconds: 1500));
      winner = WinnerMode.TIE;
      Get.find<GameAiController>().addTie();
    }
    Get.find<GameAiController>()
